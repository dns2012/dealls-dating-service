...
func TestSchemaV1API_Login(t *testing.T) {
	type input struct {
		ctx context.Context
		req *schemav1.LoginRequest
	}
	...
	tests := []testcase{
		...
		},
		{
			...
			on: func(m *SchemaV1APIMock, i *input, o *output) {
				m.loginUsecase.On("Call", i.ctx, &usecase.LoginUsecaseParams{
		...
		},
		{
			...
			on: func(m *SchemaV1APIMock, i *input, o *output) {
				m.loginUsecase.On("Call", i.ctx, &usecase.LoginUsecaseParams{
					Email:     i.req.GetEmail(),
					Password:  i.req.GetPassword(),
				}).Return(&usecase.LoginUsecaseResult{Token: o.res.Data.AccessToken}, nil)
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			m := &SchemaV1APIMock{loginUsecase: mocks.LoginUsecase{}}
			...
			subject.LoginUsecase = &m.loginUsecase
